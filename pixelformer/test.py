...
from utils import depth_value_to_depth_image, post_process_depth, flip_lr
...
parser.add_argument('--save_depth_path',            help='save prediction path', default='')
...
def test(params):
    ...
    with torch.no_grad():
        for _, sample in enumerate(tqdm(dataloader.data)):
            ...
            pred_depth = pred_depth[sample['U'].numpy().squeeze() : pred_depth.shape[0] - sample['D'].numpy().squeeze(), sample['L'].numpy().squeeze() : pred_depth.shape[1] - sample['R'].numpy().squeeze()]
            ...
            image_vis = Image.open(sample['name'][0])
            pred_vis = depth_value_to_depth_image(pred_depth)
            vis_arr = np.hstack([image_vis, pred_vis])
            vis_img = Image.fromarray(vis_arr)
            vis_path = "{}/{}".format(save_depth_path, name)
            vis_img.save(vis_path)
            if args.dataset == "kitti":       
                pred_depths.append(1 / pred_depth)
            else:
