...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        if self.mode == "test":
            ...
            image_path, depth_path, mask_path = self.samples[idx].split()
            
            mask = np.load(f"{self.args.data_path}/{mask_path}")
            
            depth_gt =  np.load(f"{self.args.data_path}/{depth_path}") #Image.open("{}/{}".format(self.args.data_path, depth_path)) #cv2.imread("{}/{}".format(self.args.data_path, depth_path), cv2.IMREAD_UNCHANGED)
        ...
        if self.mode == "train":
            ...
            sample = {'image': image, 'depth': depth_gt, 'name': full_image_path} 
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        depth = self.to_tensor(sample['depth'], name)
    ...
    def to_tensor(self, pic, name=None):
        # print(name)
        ...
        if isinstance(pic, np.ndarray):
            pic_copy = pic.copy()
            # print(pic_copy.flags['WRITEABLE'])
            img = torch.from_numpy(pic_copy.transpose((2, 0, 1)))
            return img
