...
from utils import depth_value_to_depth_image, post_process_depth, flip_lr, silog_loss, compute_errors, eval_metrics, \
...
parser.add_argument('--flip_prop',              type=float, help='horizontal flip augmentation prob', default=0.5)
parser.add_argument('--image_transform_prop',              type=float, help='image transform augmentation prob', default=0.5)
parser.add_argument('--cut_prop',              type=float, help='cut augmentation prop', default=0.75)
...
def online_eval(model, dataloader_eval, gpu, ngpus, global_step, post_process=False):
    ...
    val_dir_path = "{}{}/{}".format(args.log_directory, args.model_name, global_step)
    try:
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        ...
        # print("{} {}".format(pred_depth.shape, gt_depth.shape))
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(tqdm(dataloader.data)):
            ...
            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and not model_just_loaded:
                depth_est_vis = depth_est.repeat(1, 3, 1, 1)
                depth_gt_vis = depth_gt.repeat(1, 3, 1, 1)
                if args.dataset == 'nyu':
                    depth_est_vis = depth_est_vis / 10.
                    depth_gt_vis = depth_gt_vis / 10. 
                disp_images = torch.cat([image, depth_est_vis, depth_gt_vis], dim=3)
                disp_image_flat = torch.hstack([disp_image for disp_image in disp_images])
                disp_image_flat = disp_image_flat.detach().cpu()#.numpy()
                image_path = "{}{}/{}-{}-gpu{}.jpg".format(args.log_directory, args.model_name, args.model_name, global_step, args.gpu)
                save_image(disp_image_flat, image_path)
                time.sleep(0.1)
                model.eval()
                with torch.no_grad():
                    eval_measures = online_eval(model, dataloader_eval, gpu, ngpus_per_node, global_step, post_process=True)
...
def main():
    ...
    args.batch_size = ngpus_per_node * 4
